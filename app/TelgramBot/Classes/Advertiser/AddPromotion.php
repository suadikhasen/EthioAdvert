...
class AddPromotion
{
   ...
   public function processCommand()
   {
    if (AdvertsPostRepository::checkExistenceOfNonPaidPromotion(Chat::$chat_id))
        ...
        Chat::sendTextMessage('please send name of the promotion not greater than 20 characters');
     }
   }
    ...
    private function processQuestion($response)
   {
    ...
    private function acceptDescriptionOfAdvert(): void
    {
        if (strlen(Chat::$text_message ) > 40)
        {
            ...
            Chat::sendTextMessage('Please Send Main Message Of Promotion');
        }
    }
    ...
    private function acceptInitialDate(): void
    {
      ...
      {
        $initial_date   =  Carbon::create(Carbon::now()->year,$this->month,$this->day);
        $next_day       =  Carbon::tomorrow();
        $day_difference =  $initial_date->diffInDays($next_day,false);
        if (($day_difference >= 0) && $initial_date->diffInHours($next_days, false) >= 6)
        {
    ...
    private function acceptFinalDate():void
    {
        if ($this->validateUserInputDate())
        {
          $final_date   =  Carbon::create(Carbon::now()->year,$this->month,$this->day);
          $initial_date =  Carbon::parse($this->getInitialDate()->answer);
          $this->processFinalAndInitialDate($initial_date,$final_date);
        }else{
            Chat::sendTextMessage('please send correct date format as shown before');
        }
    }
    ...
    private function processPostAvailability(Carbon $initial_date, Carbon $final_date): void
    {
      if (GeneralService::isFreeSpaceAvailable($initial_date,$final_date))
      {
          ...
          NumberOfViewService::sendViewMessage(GeneralService::default_number_of_view,
    ...
    {
        Chat::sendTextMessage('Your Date Must Be at least six hours differ from next day');
    }
